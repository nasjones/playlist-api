...
describe('App', () => {
    ...
    describe('GET /api/playlists', () => {
        ...
        context(`Given an XSS attack playlist`, () => {
            const { maliciousPlaylist, expectedPlaylist } = fixtures.makeMaliciousPlaylist()

            beforeEach('insert malicious playlist', () => {
                return db
                    .into('playlists')
                    .insert([maliciousPlaylist])
            })
            ...
            it('removes XSS attack content', () => {
                return supertest(app)
                    ...
                    .expect(200)
                    .expect(res => {
                        expect(res.body[0].title).to.eql(expectedPlaylist.title)
                        expect(res.body[0].description).to.eql(expectedPlaylist.description)
                    })
            })
        })
    })
